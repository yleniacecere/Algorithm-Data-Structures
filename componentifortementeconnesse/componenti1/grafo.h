...
Grafo<T> Grafo<T>::trasposto() {
    ...
    for (auto nodo : grafo) {
        ...
        for (auto v : adiacenti) {
            ...
            GT.addArco(from, to); // arco invertito
        }
    }
...
void Grafo<T>::trovaSCC() {
    // Fase 1: DFS normale per riempire la stack (myQueue)
    this->dfs();

    // Fase 2: Trasposto
    Grafo<T> GT = this->trasposto();

    // Inizializza colori nel trasposto
