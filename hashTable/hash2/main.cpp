...
int main(){
  ...
  cout<<"Occorrenza del carattere '"<<searchChar
      <<"' :"<<occurrence<<endl;
  ...
  if(occurrence1 == 0){
    cout<<"Il carattere '"<< charToRemove
        <<"' è stato rimosso dalla tabella hash."<<endl;
  }else{
    cout<<"Il carattere '"<< charToRemove
        <<"' è ancora presente nella tabella hash con"
        << occurrence1<<"occorrenza(e)."<<endl;
  }
  cout<<"Tabella hash:"<<endl;
  for(const auto& bucket: myHash.getTable()){
    for(const auto& element: bucket){
